---
title: "KNN_plot_percentage_composition_change"
author: "Sven Rodne"
date: "2025-11-09"
...
relative_dominance_tph <- ggplot(species_tph_avg, aes(x = spcd, y = perc, fill = year)) +
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    plot.title = element_text(face = "bold", hjust = 0.5),
    panel.background = element_rect(fill = "white", colour = "black"), # White panel, black border
    plot.background = element_rect(fill = "white", colour = NA)        # White overall background, no border
  )
relative_dominance_tph

ggsave("Graphics/relative_dominance_tph.png", plot = relative_dominance_tph, width = 10, height = 6, dpi = 300)
```

To build the individual site plots (plug and play operation)
...
# Keyword generation for each site to plug and play into the plot and file save
# The site names for the plot = Ashland, Big Butte, Briggs Secret, Elder Creek, Elliot Ridge, Emigrant Creek, Evans Creek, Grayback Creek, Star Gulch, Taylor Ridge, Trail Creek
# The site names for the file save = ashland, big_butte, briggs_secret, elder_creek, elliot_ridge, emigrant_creek, evans_creek, grayback_creek, star_gulch, taylor_ridge, trail_creek
site_name_plug <- "Ashland"
file_name <- "ashland"

# Filter to density plots
rogue_ages <- rogue_ages %>% filter(plot_type=="density")
rogue_ages <- rogue_ages %>% filter(site_name==site_name_plug)
...
# Plotting the TPH for species for each site
...
relative_dominance_tph <- ggplot(species_tph_avg, aes(x = spcd, y = perc, fill = year)) +
  ...
  scale_fill_manual(values = c("1911" = "#2E8B57", "2011" = "#B22222")) +
  labs(
    title = paste0(site_name_plug, ": Average Relative Species Dominance by Trees per Hectare per Plot (1911 vs. 2011)"),,
    x = "Species",
    y = "Average Relative Tree Density (%)",
    fill = "Year"
  ) +
...
ggsave(paste0("Graphics/", file_name, "_relative_dominance_tph.png"), plot = relative_dominance_tph, width = 10, height = 6, dpi = 300)
```
