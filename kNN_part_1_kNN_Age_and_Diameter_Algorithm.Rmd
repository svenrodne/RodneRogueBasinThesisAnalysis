...
This serves to remove packages from this part which are used in later steps (will mess up the calculations if Rmisc and plyr are loaded)
...
# All of the possible variables used and their predictive statistics that were used
...
# Filter for true age and useful columns (this is the data set for only known ages or the knn known values)
...
# This is the run for knn on test age data (used to evaluate differing variables)
...
# knn run on the actual full dataset
...
# Calculate and add the 1911 diameters from cored trees using chronology information
...
filtered_mastall_data <- collapsed_mastall %>%
  select(year, any_of(valid_trees$rodne_sid))
...
# All possible variables that were tested for predicting tree diameter
...
# Go to hardwood resprouter (part 2)
